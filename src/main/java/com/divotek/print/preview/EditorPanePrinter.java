...
public class EditorPanePrinter extends JPanel implements Pageable, Printable {
    ...
    public void initData(JEditorPane pane, Paper paper, Insets margins) {
        ...
        this.paper.setImageableArea(0,0,paper.getWidth(), paper.getHeight());
...
//        sourcePane.setSize(Integer.MAX_VALUE, pageHeight);
//        Dimension d=sourcePane.getPreferredSize();
//        sourcePane.setSize(d.width, pageHeight);


        calculatePageInfo();
        int count=pages.size();
	  //pageWidth*2+50
        this.setPreferredSize(new Dimension(pageWidth,PAGE_SHIFT+count*(pageHeight+PAGE_SHIFT)));
    }
	...
	//	pageFormat.setOrientation(PageFormat.LANDSCAPE);
	//	pageFormat.setPaper(paper);
//	PageFormat tm = new PageFormat();
//	tm = 	(PageFormat) pageFormat.clone();
//	Paper p = new Paper();
//	p=(Paper) paper.clone();
//	p.setSize(20, 20);
//	tm.setPaper(p);
